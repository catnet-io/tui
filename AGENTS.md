...
10. **No Automatic Merges to `main`.** Never perform an automatic merge to `main`. All merges to `main` must be manually reviewed, approved, and executed by human maintainers.
11. **No Downgrades.** Dependency and GitHub Action versions must never be downgraded. Upgrades to newer stable versions must be preserved, and changes or PRs attempting to revert or downgrade existing dependency or GitHub Action versions are strictly forbidden across all repositories.
